...
对比分支:
- baseline_v2    -> 640-d global concat
- icl_stats_v2   -> 640 + ICL2/3 stats + family_pos_rate + icl_sim (658-d)
- icl_full_v2    -> 640 + ICL2/3 full + family_pos_rate + icl_sim (1,298-d)
"""
...
OUTPUT_FILE = DATA_DIR / "paired_cv_enhanced_v2_results.json"
...
def compute_icl_similarity(gid, icl_data, human_references):
    ...
    def get_icl_seq(r):
        ...
        # 这里我们只有 boundaries，没有原始 sequence
        # 从 stats 反推长度，用零填充... 不够好
        # 改用 ICL2/3_stats 向量的 cosine similarity
        s2 = r.get("ICL2_stats", {})
        s3 = r.get("ICL3_stats", {})
        vec = []
...
def build_vectors(df, gpcr_feats, gprot_feats, icl_data, family_map, family_rates, global_rate,
...
def main():
    ...
    all_results = {}
    for mode in ["baseline", "icl_stats", "icl_full", "icl_stats_v2", "icl_full_v2"]:
        print(f"\n{'='*70}")
        print(f"  模式: {mode}")
        print(f"{'='*70}")
        X, y, meta = build_vectors(df, gpcr_feats, gprot_feats, icl_data,
