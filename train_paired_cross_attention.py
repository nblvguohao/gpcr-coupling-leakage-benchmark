#!/usr/bin/env python3
"""
Cross-attention neural network training on the 1,639 paired dataset.
Supports cluster-aware cross-validation for fair comparison with SVM baselines.
"""
...
OUTPUT_FILE = DATA_DIR / "paired_cross_attention_results.json"

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

# ---------------------------------------------------------------------------
# Feature loading utilities (same logic as paired_cross_validation_enhanced.py)
# ---------------------------------------------------------------------------
...
class PairedCrossAttentionNet(nn.Module):
    def __init__(self, gpcr_dim=320, gprot_dim=320, hidden_dim=256, num_heads=4, dropout=0.3):
        super().__init__()
    ...
    def forward(self, gpcr_feat, gprot_feat):
        # gpcr_feat: (B, gpcr_dim), gprot_feat: (B, gprot_dim)
...
class PairDataset(Dataset):
    def __init__(self, X, y, gpcr_total_dim):
        ...
        self.gpcr_total_dim = gpcr_total_dim  # includes GPCR ESM + ICL + AlphaFold
    ...
    def __getitem__(self, idx):
        x = self.X[idx]
        # First gpcr_total_dim is GPCR-specific; positions 320:640 are always G-protein ESM
        gpcr_feat = x[:self.gpcr_total_dim]
        gprot_feat = x[320:640]
...
def run_cluster_cv(X, y, meta, cluster_list, mode_name, n_folds=5, epochs=80, lr=1e-4, batch_size=32):
    ...
    # GPCR-specific total dim: ESM 320 + optional ICL 656 + optional AlphaFold 15
    # G-protein dim is always 320 (positions 320:640 in the concatenated vector)
    if mode_name == "baseline":
        gpcr_total_dim, gprot_dim = 320, 320
    elif mode_name == "icl_full":
        gpcr_total_dim, gprot_dim = 320 + 656, 320
    elif mode_name == "alpha":
        gpcr_total_dim, gprot_dim = 320 + 656 + 38, 320
    else:
...
def main():
    print("=" * 70)
    print("  Cross-Attention Training on Paired Dataset (1,639 pairs)")
