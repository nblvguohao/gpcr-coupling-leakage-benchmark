...
\begin{itemize}[leftmargin=*,itemsep=4pt]
  \item GPCR--G protein coupling is reformulated as a pairwise binary classification over 1,647 experimentally validated (GPCR, G protein family) pairs, incorporating G protein identity as explicit input.
  \item Cross-attention with ESM-2 650M embeddings and topology-aware intracellular loop features achieves AUC of 0.862 under cluster-aware cross-validation, with a Brier score of 0.008 indicating well-calibrated probabilities.
  \item A critical methodological finding: intracellular loop local features must match the global embedding dimension---mixing heterogeneous dimensions degrades performance across all architectures.
  \item AlphaFold structural descriptors provide no incremental gain beyond ESM-2 650M embeddings, demonstrating that high-capacity protein language models implicitly encode sufficient structural information.
  \item A companion software tool (\texttt{gpcr-coupling}) with pre-trained models and a tutorial workflow is provided for practical deployment.
\end{itemize}
\end{tcolorbox}
...
\textbf{Availability:} Source code, pre-trained models, and datasets are available at \url{https://github.com/nblvguohao/gpcr-coupling-leakage-benchmark} and archived on Zenodo. The companion tool \texttt{gpcr-coupling} provides a command-line interface for prediction and feature extraction.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.88\textwidth]{figures/figure1_auc_comparison.pdf}
\caption{Cluster-aware CV AUC across all model configurations. Cross-attention with 650M embeddings and ICL features achieves the highest AUC (0.8619). The performance advantage of cross-attention over SVM widens from $\Delta$AUC $= +0.006$ at 8M to $+0.029$ at 650M, indicating better utilization of high-dimensional representations.}
\label{fig:auc}
\end{figure}
...
\caption{ICL dimension alignment effect. Heterogeneous feature dimensions systematically degrade performance, while dimension-matched features enable cross-attention to fully utilize high-dimensional embeddings.}
\label{fig:icl}
\end{figure}
...
All data, code, and pre-trained models supporting this study are available at \url{https://github.com/nblvguohao/gpcr-coupling-leakage-benchmark} under the MIT license, and are permanently archived on Zenodo (DOI: 10.5281/zenodo.XXXXXXX). The paired dataset (1,647 pairs, 431 GPCRs) is provided in CSV format with corresponding FASTA sequences. Pre-computed ESM-2 embeddings (8M and 650M) for all GPCRs and G proteins, ICL features, and AlphaFold structural descriptors are included to enable reproduction without GPU requirements for feature extraction. Cross-validation fold assignments and cluster membership files are provided to ensure exact reproducibility of all reported metrics.
