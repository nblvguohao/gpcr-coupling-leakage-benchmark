...
\begin{itemize}
  ...
  \item Cross-attention + ESM-2 650M embeddings + topology-aware ICL features achieves AUC 0.862 (cluster-aware CV), with Brier score 0.008 indicating well-calibrated, deployment-ready probabilities.
  \item Critical dimension-alignment finding: ICL local features must match global embedding dimension---mixing dimensions degrades performance across all architectures.
  \item AlphaFold structural descriptors provide zero incremental gain beyond ESM-2 650M, demonstrating that large protein language models implicitly encode structural information.
