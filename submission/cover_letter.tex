...
\begin{itemize}
  ...
  \item Thirty-eight standard AlphaFold structural descriptors provide no incremental benefit beyond ESM-2 650M embeddings ($\Delta$AUC $\le$ 0.002), demonstrating that these PLM representations already capture structurally relevant information for family-level coupling classification.
...
\begin{enumerate}
    \item \textbf{Systematic benchmarking:} First comparison of SVM, MLP, Random Forest, XGBoost, and cross-attention architectures for GPCR coupling prediction under three evaluation protocols (random CV, cluster-aware CV, leave-one-G-protein-family-out).
